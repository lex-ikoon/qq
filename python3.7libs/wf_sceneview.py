...
def toggle_points_discs () :
    panetab_cursor = hou.ui.curDesktop().paneTabUnderCursor()
    particleType   = panetab_cursor.curViewport().settings().particleDisplayType()
    print (particleType)
    if particleType != hou.viewportParticleDisplay.Discs :
        typeNew = hou.viewportParticleDisplay.Discs
        status  = "ON"
    else :
        typeNew = hou.viewportParticleDisplay.Points
        status  = "OFF"

    panetab_cursor.curViewport().settings().particleDisplayType(typeNew)
    message = "visualise Discs : " + status
    print (message)
...
def bring_center_to_view () :
    panetab_cursor = hou.ui.curDesktop().paneTabUnderCursor()
    
    camera   = panetab_cursor.curViewport().defaultCamera()
    node     = panetab_cursor.currentNode()
    camPivot = camera.pivot()


    # object type >>> parm name
    #   transform: px py pz (camPivot translate)
    #        line: originx originy originz
    #   all other: tx ty tz
    parmName = "t"
    if node.type() == hou.nodeType("Sop/xform") :
        parmName = "p"
    if node.type() == hou.nodeType("Sop/line") :
        parmName = "origin"


    # eval the parm
    parmEval      = node.parmTuple(parmName).eval()
    # if node is Transform SOP, then subtract tx from the px
    if node.type() == hou.nodeType("Sop/xform") :
        parmTxyz   = node.parmTuple("t").eval()
        camOffsetx = camPivot[0] - parmTxyz[0]
        camOffsety = camPivot[1] - parmTxyz[1]
        camOffsetz = camPivot[2] - parmTxyz[2]
        camPivot   = (camOffsetx, camOffsety, camOffsetz)


    # viewport type
    # if ortho, then move only in XY, XZ or YZ
    viewportType = panetab_cursor.curViewport().type()
    if viewportType == hou.geometryViewportType.Right or viewportType == hou.geometryViewportType.Left :
        parmNew =  (  parmEval[0]   ,  camPivot[1]  ,  camPivot[2]  )
    if viewportType == hou.geometryViewportType.Top or viewportType == hou.geometryViewportType.Bottom :
        parmNew =  (  camPivot[0]   ,  parmEval[1]  ,  camPivot[2]  )
    if viewportType == hou.geometryViewportType.Front or viewportType == hou.geometryViewportType.Back :
        parmNew =  (  camPivot[0]   ,  camPivot[1]  ,  parmEval[2]  )
    if viewportType == hou.geometryViewportType.Perspective :
        parmNew =  camPivot


    # set the parm
    node.parmTuple(parmName).set(parmNew)
...
def toggle_opencurves () :
    panetab_cursor = hou.ui.curDesktop().paneTabUnderCursor()
    open_curves    = panetab_cursor.curViewport().settings().shadeOpenCurves()
    panetab_cursor.curViewport().settings().shadeOpenCurves(not open_curves)
    ...
    message = "visualise OpenCurves : " + status
    hou.ui.setStatusMessage(message)


def toggle_opencurves_geo () :
    panetab_cursor = hou.ui.curDesktop().paneTabUnderCursor()
    open_curves    = panetab_cursor.curViewport().settings().shadeOpenCurves()
    panetab_cursor.curViewport().settings().shadeOpenCurves(not open_curves)
