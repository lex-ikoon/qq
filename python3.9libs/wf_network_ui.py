...
def qq_expand (node,parm_code) :
    ...
    path_shortcuts = hou.getenv("wf_path") + "/vex/include/triggers.db"
    path_snippets  = hou.getenv("wf_path") + "/vex/include/snippets.db"

    with open(path_shortcuts, 'rb') as f:
        src = pickle.load(f)
...
def qq_parse_uberfile () :
    ...
    path_VEX  = hou.getenv("wf_path") + "/vex/include/"
    ...
    file_shortcuts = open( path_shortcuts, 'wb')
...
def folders_tab_go (dir) :
    ...
    if node.type() == hou.nodeType("Sop/switch") or node.type() == hou.nodeType("Dop/switch") or node.type() == hou.nodeType("Lop/switch") or node.type() == hou.nodeType("Chop/switch") or node.type() == hou.nodeType("Top/switch"):
        ...
        ########################
        #######  blend  #######
        colorActive = hou.Color(1.00, 1.00, 1.00)
        colorGrey   = hou.Color(0.25, 0.25, 0.25)
        for blend in hou.selectedNodes():
            in0  = blend.input( 0 )
            in1  = blend.input( 1 )
            parm = blend.parm("sequence_seq")

            if parm.eval() == 0 : 
                parm.set( 1 )
                in0.setColor(colorGrey)
                in1.setColor(colorActive)
            else:
                parm.set( 0 )
                in0.setColor(colorActive)
                in1.setColor(colorGrey)
